...
const CONFIG = {
  ...
  renderedWidth: 64,
  renderedHeight: 64,
  spriteSheet: "/images/sprites/duck.webp",
  animations: {
    walking: {
      row: 0,
      frames: 5,
...
export default function DuckFollowerCore() {
  ...
  useEffect(() => {
    ...
    image.onload = () => {
      if (isMounted) setHasSpriteSheet(true)
    }

    image.onerror = () => {
      if (isMounted) setHasSpriteSheet(false)
    }

    image.src = CONFIG.spriteSheet
  ...
  if (!hasSpriteSheet) {
    return (
      <div
        className="duck-follower rounded-full bg-amber-300 px-2 py-1 text-[10px] leading-none font-semibold text-amber-950"
        style={{ transform }}
        aria-hidden="true"
      >
        DUCK
